...
std::vector<float> downmix_interleaved_to_mono(const std::vector<float>& interleaved, const int channels) {
    ...
    for (std::size_t frame = 0; frame < frames; ++frame) {
        ...
        for (int ch = 0; ch < channels; ++ch) {
            sum += static_cast<double>(interleaved[base + static_cast<std::size_t>(ch)]);
        }
...
std::vector<float> normalize_to_session_audio(
...
std::vector<float> slice_audio_range(
...
std::vector<float> resample_linear_to_size(const std::vector<float>& input, const std::size_t target_size) {
    ...
    std::vector<float> out(target_size, 0.0f);
...
std::vector<float> extract_with_offset(
    const std::vector<float>& input,
    const std::size_t offset_samples,
    const std::size_t target_samples) {
    if (target_samples == 0) {
        return {};
    }
    std::vector<float> out(target_samples, 0.0f);
    if (offset_samples >= input.size()) {
        return out;
    }
...
std::vector<float> mono_to_interleaved_channels(const std::vector<float>& mono, const int channels) {
    ...
    std::vector<float> interleaved(mono.size() * static_cast<std::size_t>(channels), 0.0f);
    for (std::size_t i = 0; i < mono.size(); ++i) {
        const std::size_t base = i * static_cast<std::size_t>(channels);
        for (int ch = 0; ch < channels; ++ch) {
            interleaved[base + static_cast<std::size_t>(ch)] = mono[i];
        }
    }
    return interleaved;
}
...
void Session::remove_track(const std::int64_t track_id) {
    const auto it = std::find_if(tracks_.begin(), tracks_.end(), [&](const TrackState& track) {
...
void Session::import_audio_to_track(
    ...
    if (!track) {
        throw std::invalid_argument("track_id does not exist");
    }

    const auto session_audio = normalize_to_session_audio(interleaved_samples, sample_rate, channels);
    track->duration_seconds = session_audio.empty()
        ? 0.0
        : (static_cast<double>(session_audio.size()) / static_cast<double>(kSessionSampleRate));
    ...
    for (std::size_t i = 0; i < blobs.size(); ++i) {
        ...
        blob.link_prev = (i > 0) ? std::optional<std::int64_t>(blob.id - 1) : std::nullopt;
        blob.link_next = (i + 1 < blobs.size()) ? std::optional<std::int64_t>(blob.id + 1) : std::nullopt;
        ...
        if (!source_range.is_valid() || source_range.length() <= 0.0) {
            source_range = blob.time;
        }
...
void Session::insert_audio_to_track(
    const std::int64_t track_id,
    const std::vector<float>& interleaved_samples,
    const int sample_rate,
    const int channels,
    const double insert_time_seconds) {
    auto* track = find_track(track_id);
    if (!track) {
        throw std::invalid_argument("track_id does not exist");
    }
    ...
    std::int64_t next_blob_id = 1;
    for (const auto& existing : track->blobs) {
        next_blob_id = std::max(next_blob_id, existing.id + 1);
    }
    ...
    for (std::size_t i = 0; i < blobs.size(); ++i) {
        ...
        if (!local_source.is_valid() || local_source.length() <= 0.0) {
            local_source = blob.time;
        }
    ...
    std::sort(track->blobs.begin(), track->blobs.end(), [](const core::NoteBlob& a, const core::NoteBlob& b) {
        if (a.time.start_seconds != b.time.start_seconds) {
            return a.time.start_seconds < b.time.start_seconds;
        }
        return a.id < b.id;
    });
    ...
    for (const auto& blob : inserted) {
        track->rendered_blob_cache.erase(blob.id);
    }
    track->render_units = planner_.plan(track->blobs);
    refresh_derived_dirty(*track);
}

std::int64_t Session::import_audio_as_new_track(
    const std::vector<float>& interleaved_samples,
    const int sample_rate,
    const int channels,
...
void Session::set_track_gain_db(const std::int64_t track_id, const double gain_db) {
    ...
    if (!std::isfinite(gain_db)) {
        throw std::invalid_argument("gain_db must be finite");
    }
...
bool Session::is_unit_dirty(const TrackState& track, const render::RenderUnit& unit) const {
    for (const auto note_id : unit.note_ids) {
        ...
        if (cache_it == track.rendered_blob_cache.end()) {
            return true;
        }
        if (cache_it->second.source_revision != live->edit_revision) {
            return true;
        }
    }
    return false;
...
void Session::refresh_derived_dirty(TrackState& track) {
    track.dirty_timeline.reset(track.duration_seconds);
    for (const auto& unit : track.render_units) {
        if (is_unit_dirty(track, unit)) {
            track.dirty_timeline.mark_dirty(unit.span);
        }
    }
}
...
std::vector<TrackRenderPlan> Session::plan_render_from(const double playhead_seconds, const std::size_t budget_units_per_track) const {
    ...
    for (const auto& track : tracks_) {
        ...
        if (!units.empty()) {
            plans.push_back(TrackRenderPlan {.track_id = track.id, .units = std::move(units)});
        }
    }
    return plans;
}
    ...
    for (const auto& plan : plans) {
        ...
        for (const auto& unit : plan.units) {
            ...
            for (const auto note_id : unit.note_ids) {
                ...
                if (live_blob) {
                    live_blobs.push_back(live_blob);
                }
            }
            if (live_blobs.empty()) {
                continue;
            ...
            if (all_unedited) {
                for (const auto* live_blob : live_blobs) {
                    ...
                    if (target_samples > 0 && rendered.size() != target_samples) {
                        rendered = resample_linear_to_size(rendered, target_samples);
                    }
            ...
            for (const auto* b : live_blobs) {
                group_blobs.push_back(*b);
            }
        ...
        while (!track.dirty_timeline.dirty_ranges().empty()) {
            ...
            if (after >= before) {
                ...
                if (stagnant_rounds >= 4) {
                    throw std::runtime_error("render_all_dirty stalled: dirty ranges not shrinking");
                }
            } else {
                stagnant_rounds = 0;
            }
...
bool Session::is_track_active_in_mix(const TrackState& track) const {
    const bool has_solo = std::any_of(tracks_.begin(), tracks_.end(), [](const TrackState& t) {
        return t.mix.solo;
    });
...
std::vector<float> Session::build_track_audio(const std::int64_t track_id, const bool apply_track_gain) const {
    ...
    for (const auto& blob : track->blobs) {
        ...
        if (it == track->rendered_blob_cache.end()) {
            continue;
        }
    ...
    if (apply_track_gain) {
        apply_gain_db(mixed, track->mix.gain_db);
    }
    for (auto& s : mixed) {
...
std::vector<float> Session::build_mixdown() const {
    ...
    for (const auto& track : tracks_) {
        if (!is_track_active_in_mix(track)) {
            continue;
        }
    ...
    for (const auto& audio : active_tracks) {
...
void Session::export_audio(const ExportRequest& request) const {
    if (request.format.sample_rate <= 0) {
        throw std::invalid_argument("export sample_rate must be positive");
    }
    if (request.format.channels <= 0) {
        throw std::invalid_argument("export channels must be positive");
    }
    ...
    if (request.mode == ExportMode::Mixdown) {
        if (request.output_path.empty()) {
            throw std::invalid_argument("mixdown output_path is empty");
        }
    ...
    if (request.stems_directory.empty()) {
        throw std::invalid_argument("stems_directory is empty");
    }
    std::filesystem::create_directories(request.stems_directory);
    for (const auto& track : tracks_) {
        if (request.stems_respect_mute_solo && !is_track_active_in_mix(track)) {
            continue;
        }
...
void Session::restore_project_state(const project::ProjectState& state) {
    if (state.session_sample_rate != kSessionSampleRate) {
        throw std::invalid_argument("project state session_sample_rate mismatch");
    }
...
double Session::duration_seconds() const {
    double d = 0.0;
    for (const auto& track : tracks_) {
        d = std::max(d, track.duration_seconds);
    }
    return d;
}
