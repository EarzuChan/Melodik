...
namespace {
    ...
    float midi_to_hz(const double midi) {
        if (midi <= 0.0) return 0.0f;
        return 440.0f * std::pow(2.0f, static_cast<float>((midi - 69.0) / 12.0));
    }

    double midpoint_seconds(const double a, const double b) { return a + (b - a) * 0.5; }

    double frame_start_seconds(const melodick::core::PitchSlice& f0, const std::size_t index) {
        if (f0.empty() || index >= f0.size()) return 0.0;
        if (index == 0) {
            if (f0.size() == 1) return std::max(0.0, f0.front().seconds);
    ...
    double frame_end_seconds(const melodick::core::PitchSlice& f0, const std::size_t index) {
        if (f0.empty() || index >= f0.size()) return 0.0;
        if (index + 1 < f0.size()) return midpoint_seconds(f0[index].seconds, f0[index + 1].seconds);
        if (index == 0) return std::max(0.0, f0.front().seconds);
    ...
    double span_duration_seconds(const melodick::core::PitchSlice& f0, const FrameSpan& span) {
        if (span.start_index > span.end_index || span.end_index >= f0.size()) return 0.0;
    ...
    void absorb_short_unvoiced_gaps_into_following_voiced_notes(
        ...
        for (std::size_t note_index = 1; note_index < notes.size(); ++note_index) {
            ...
            if (gap_duration <= 1.0e-9 || gap_duration > config.max_leading_unvoiced_seconds) continue;
            ...
            for (std::size_t frame_index = 0; frame_index < f0.size(); ++frame_index) {
                ...
                if (const double frame_end = frame_end_seconds(f0, frame_index); frame_end <= gap_start || frame_start >= gap_end) continue;
                prefix.push_back(f0[frame_index]);
            }
            if (prefix.empty()) continue;

            const bool all_unvoiced = std::ranges::all_of(prefix, [](const auto& point) { return !point.voiced; });
            if (!all_unvoiced) continue;
    ...
    void donate_long_unvoiced_edges_to_neighboring_spans(
        ...
        const melodick::core::PitchSlice& f0,
        const melodick::capabilities::SegmenterConfig& config) {
        if (spans.empty() || f0.empty()) { return; }

        for (std::size_t i = 0; i < spans.size(); ++i) {
            auto& span = spans[i];
            if (span.voiced || span_duration_seconds(f0, span) <= config.max_leading_unvoiced_seconds) { continue; }

            const bool has_prev_voiced = i > 0 && spans[i - 1].voiced;
            const bool has_next_voiced = i + 1 < spans.size() && spans[i + 1].voiced;
            if (!has_prev_voiced && !has_next_voiced) { continue; }

            const double old_start = frame_start_seconds(f0, span.start_index);
            const double old_end = frame_end_seconds(f0, span.end_index);
            const double prefix_end = has_prev_voiced ? std::min(old_end, old_start + config.long_unvoiced_prefix_to_prev_seconds) : old_start;
            const double suffix_start = has_next_voiced ? std::max(prefix_end, old_end - config.long_unvoiced_suffix_to_next_seconds) : old_end;
            ...
            if (has_next_voiced) {
                for (std::size_t frame_index = middle_start; frame_index <= span.end_index; ++frame_index) {
                    if (frame_start_seconds(f0, frame_index) >= (suffix_start - 1.0e-9)) {
                        ...
                        if (frame_index == 0) { middle_end = 0; }
                        else { middle_end = frame_index - 1; }
                        break;
                    }
                }
            }
...
namespace melodick::capabilities {
    ...
    std::vector<core::NoteBlob> NoteBlobSegmenter::build_segments(const core::PitchSlice& f0) const {
        ...
        for (std::size_t i = 1; i < f0.size(); ++i) {
            if (f0[i].voiced == f0[i - 1].voiced) { continue; }
        ...
        // then longer unvoiced spans donate edge context (30ms to prev, 60ms to next).
        donate_long_unvoiced_edges_to_neighboring_spans(compact_spans, f0, config_);

        for (const auto& span : compact_spans) { append_blob(span); }

        absorb_short_unvoiced_gaps_into_following_voiced_notes(notes, f0, config_);

        for (std::size_t i = 0; i < notes.size(); ++i) {
            if (i > 0 && notes[i - 1].has_voiced_content() && notes[i].has_voiced_content()) { notes[i].link_prev = notes[i - 1].id; }
            if (i + 1 < notes.size() && notes[i].has_voiced_content() && notes[i + 1].has_voiced_content()) { notes[i].link_next = notes[i + 1].id; }
        }
